...
export async function get_photo(photo_data, res, isCurrentPhoto = false) {    
    ...
    // Sanitize error message for HTTP headers
    const sanitizeErrorMessage = (msg) => msg.replace(/\n/g, ' | ').replace(/\r/g, '').trim();
    ...
    fs.readFile(photo_data.filename, (err, data) => {
        if (err) {
            ...
            const errorMsg = `Failed to read photo file. | Filename: ${path.basename(photo_data.filename)} | Path: ${photo_data.filename} | Folder: ${photo_data.meta_data?.folder_name || 'Unknown'} | Error: ${err.code === 'ENOENT' ? 'File not found' : err.message}`;
            res.writeHead(500, { 
              'Content-Type': 'image/jpeg',
              'X-Image-Status': 'error',
              'X-Error-Message': sanitizeErrorMessage(errorMsg)
            });
            // Send a minimal default image data (1x1 transparent JPEG)
